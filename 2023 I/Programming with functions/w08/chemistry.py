...
"C10H8": "naphthalene",
"C12H22O11": "sucrose",
"C12H22O11·H2O": "invert sugar",
"C16H18N3SCl": "methylene blue",
"C18H15Cl2N3O2": "dicamba",
"C18H20FN3O4S": "linezolid",
"C19H21NO4": "papaverine",
}


def main():
    chem_formula = input("Enter the molecular formula of the sample: ").strip()
    sample_mass = input("Enter the mass in grams of the sample: ")
    periodic_table = make_periodic_table()
    parsed_chem_formula = parse_formula(chem_formula, periodic_table)
    molar_mass = compute_molar_mass(parsed_chem_formula, periodic_table)
    moles = float(sample_mass) / molar_mass
    print(f"{round(molar_mass, 5)} grams/mole")
    print(f"{round(moles, 5)} moles")
    print(f"Compound Name: {get_formula_name(chem_formula, known_molecules_dict).title()}")
    print(f"The total number of protons is: {sum_protons(parsed_chem_formula, periodic_table)}")
...
def sum_protons(symbol_quantity_list, periodic_table_dict):
    ...
    for element in symbol_quantity_list:
        ...
        total_protons += quantity * atomic_number
    return total_protons

def make_periodic_table():
    """Creates and returns a compound list of elements molar masses. 

    Parameter
        None
    Return: Array of elements periodic_table_list
    """
    periodic_table_dict = {
